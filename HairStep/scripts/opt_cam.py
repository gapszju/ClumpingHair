...
def opt_cam(opt):
    ...
    items = [item for item in os.listdir(img_dir) if not item.endswith('Thumbs.db')]

    sample_info_list = [(img_dir +'/'+ item, lmk_dir +'/'+ item[:-3]+'npy', 
                    param_dir +'/'+ item[:-3]+'npy', lmk_proj_dir +'/'+ item) for item in items]

    for sample_info in tqdm(sample_info_list):
