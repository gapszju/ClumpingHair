...
// Given a normalized device coordinate -range/2 <= x < range/2, convert it to a pixel
// coordinates. 
__device__ inline int NonSquareNdcToPix(float x, int S1, int S2) {
  float range = NonSquareNdcRange(S1, S2);
  // The NDC range is [-range/2, range/2].
  float offset = (range / 2.0f);
  return __floorf((x + offset) / range * S1);
}
...
const int32_t kMaxPointsPerPixel = 512;
