...
class CurvesRenderer(nn.Module):
    """
    A class for rendering a batch of curves. The class should
    ...
    def forward(self, curves, **kwargs) -> torch.Tensor:
        fragments = self.rasterizer(curves, **kwargs)

        # Construct weights based on the distance of a point to the true point.
        # However, this could be done differently: e.g. predicted as opposed
        # to a function of the weights.
        blur_radius = self.rasterizer.raster_settings.blur_radius
        image = self.compositor(fragments, blur_radius)
