...
def load_hairstep(orien2d_path, depth_path, seg_path, load_size=512, depth_out_mask=-3.0):
    ...
    depth = depth + out_mask*depth_out_mask
...
def recon3D_from_hairstep(opt):
    print("reconstruct 3D hair from hair step map")
    ...
    items = [item for item in os.listdir(strand_dir) if not item.endswith('Thumbs.db')]

    for item in tqdm(items):
        strand_path = os.path.join(strand_dir, item[:-3] + 'png')
        seg_path = os.path.join(opt.root_real_imgs, 'seg', item[:-3] + 'png')
        calib_path = os.path.join(opt.root_real_imgs, 'param', item[:-3] + 'npy')
        depth_path = os.path.join(opt.root_real_imgs, 'depth_map', item[:-3] + 'npy')
