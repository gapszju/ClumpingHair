...
class OptLandmark(nn.Module):
    def __init__(self, lmk_gt_path, input_img_path, width=512):
        ...
        self.register_buffer('scale', torch.Tensor([363.1]))
        self.register_buffer('center', torch.Tensor([[-0.001],[1.64],[-0.03]]))
        self.register_buffer('rotation', torch.Tensor([[0.9, 0., -0.04], [0.001, 0.99, 0.023], [0.037, -0.026, 0.89]]))
        
        self.scale_dis = nn.Parameter(torch.zeros_like(self.scale))
        self.center_dis = nn.Parameter(torch.zeros_like(self.center))
        self.rotation_dis = nn.Parameter(torch.zeros_like(self.rotation))
    ...
    def get_camera(self):
        ...
        # model rotation
        R =  self.rotation_dis+self.rotation
